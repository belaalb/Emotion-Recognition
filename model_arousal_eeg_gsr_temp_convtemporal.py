...
class model(nn.Module):
	...
	def __init__(self):
		...
		self.fc1_eeg = nn.Linear(8*7*4, 40)
		self.fc2_eeg = nn.Linear(40, 30)					# Representation to be concatenated
		...
		self.fc2_temp_gsr = nn.Linear(32, 25)				# Representation to be concatenated
				

		# Output layer
		self.fc_out_1 = nn.Linear(55, 30)
		self.fc_out_2 = nn.Linear(30, 10)
		self.fc_out_3 = nn.Linear(10, 5)
		self.fc_out_arousal = nn.Linear(5, 2)
	...
	def forward_multimodal_arousal(self, x):
		...
		output = F.alpha_dropout(output, training = self.training)		
		output = F.selu(output)
		output = self.fc_out_3(output)
		output = F.alpha_dropout(output, training = self.training)		
		output = F.selu(output)
		output = F.softmax(self.fc_out_arousal(output))
